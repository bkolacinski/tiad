...
"%PYTHON_EXE%" -m pip install --upgrade pip "setuptools<82" wheel --trusted-host pypi.org --trusted-host files.pythonhosted.org
if errorlevel 1 (
...
"%PYTHON_EXE%" -m pip install -r requirements.txt --trusted-host pypi.org --trusted-host files.pythonhosted.org
if errorlevel 1 (
...
echo [3/4] Installing PyInstaller...
"%PYpause
    THON_EXE%" -m pip install pyinstaller --quiet
if errorlevel 1 (
    echo ERROR: Failed to install PyInstaller
    exit /b 1
)
